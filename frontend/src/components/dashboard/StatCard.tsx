...
export default function StatCard({ title, value, index = 0 }: StatCardProps) {
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl ${config.accentColor} rounded-bl-full pointer-events-none opacity-60 transition-opacity group-hover:opacity-100`}
      />
      ...
      <div className="flex justify-between items-start relative z-10">
        ...
        <div
          className={`p-3 rounded-xl ${config.badgeBg} transition-transform duration-300 group-hover:scale-110 shadow-sm`}
        >
          {config.icon}
        </div>
      </div>
