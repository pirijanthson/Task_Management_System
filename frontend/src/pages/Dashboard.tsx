      ...
      >
        <div>
          ...
          <p className="text-blue-100 dark:text-slate-300 text-sm mt-1 max-w-xl">
            Monitor your daily task statistics, progress, and pending deadlines in real time.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <StatCard title="Total Tasks" value={stats.totalTasks} color="border-blue-500" index={0} />
            <StatCard title="Pending" value={stats.pendingTasks} color="border-yellow-500" index={1} />
            <StatCard title="In Progress" value={stats.inProgressTasks} color="border-indigo-500" index={2} />
            <StatCard title="Completed" value={stats.completedTasks} color="border-emerald-500" index={3} />
          </div>

          {/* Secondary Grid: Progress & Overdue Focus */}
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 text-center">
                <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <p className="text-lg font-bold text-amber-600 dark:text-amber-400">{stats.pendingTasks}</p>
                </div>
                <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
                  <p className="text-xs text-slate-400 font-medium">Active</p>
                  <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{stats.inProgressTasks}</p>
                </div>
                <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
                  <p className="text-xs text-slate-400 font-medium">Done</p>
                  <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{stats.completedTasks}</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/50">
                  <div>
                    <span className="text-xs font-semibold text-rose-700 dark:text-rose-300 uppercase tracking-wider">Overdue Tasks</span>
